...
// Check if API key is missing and provide a helpful error message
if (!API_KEY) {
  console.error("ERROR: GEMINI_API_KEY environment variable is not set!");
  console.error("Please add your Gemini API key as an environment variable.");
  console.error("You can get one from: https://aistudio.google.com/app/apikey");
}
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/health", (req, res) => {
    ...
    const aiStatus = API_KEY ? "available" : "unavailable";
    ...
    try {
      ...
      if (!API_KEY) {
        return res.status(503).json({ 
          ...
          error: "GEMINI_API_KEY environment variable is not set. Please configure it in your deployment environment." 
        });
      }
